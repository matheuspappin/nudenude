  ...
  return (
    <div className="flex flex-col gap-8 relative">
      <div className="flex items-center justify-between">
        ...
        <button className="text-sm font-medium bg-white/5 border border-white/10 hover:bg-white/10 text-white px-4 py-2 rounded-lg transition-colors">
          Configure Prices
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="p-6 bg-card border border-white/10 rounded-xl shadow-sm flex flex-col justify-between">
        ...
        <div className="p-6 bg-card border border-white/10 rounded-xl shadow-sm flex flex-col justify-between">
          ...
          <div className="mt-4">
            <p className="text-3xl font-bold text-white tracking-tighter">184</p>
            <p className="text-xs text-emerald-400 font-semibold mt-3">+12 new this week</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-card border border-white/10 rounded-xl shadow-sm flex flex-col justify-between">
          ...
          <div className="mt-4">
            <p className="text-3xl font-bold text-white tracking-tighter">$1,838.16</p>
            <p className="text-xs text-zinc-500 font-medium mt-3">Based on VIP Tier $9.99</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 bg-card border border-white/10 rounded-xl h-72 flex flex-col items-center justify-center border-dashed text-zinc-500">
         ...
         <p className="text-sm">Subscriber Retention Chart</p>
         <p className="text-xs text-zinc-600 mt-1">Connect analytics API to visualize.</p>
      </div>
          ...
          <div className="bg-card border border-white/10 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleWithdraw} className="p-6 flex flex-col gap-5">
              ...
              <div className="mt-4 pt-4 border-t border-white/5 flex justify-end gap-3">
                ...
                <button type="submit" disabled={isProcessing || !withdrawAmount || parseFloat(withdrawAmount) > balance} className="px-6 py-2 rounded-lg font-bold text-sm bg-primary text-primary-foreground shadow-glow hover:bg-primary/90 transition-all disabled:opacity-50 flex items-center justify-center">
                  {isProcessing ? (
